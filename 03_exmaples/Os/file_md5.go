...
func main() {
	...
	if _, err := os.Stat(filename); os.IsNotExist(err) {
		...
		file, err = os.OpenFile(filename, os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0777) //openFile 可以设置读写权限，open函数只能是只读权限
	}
	defer file.Close()
	//文件写入内容
	_, err := io.WriteString(file, "这是写入的字符串") //也可以使用file.WriteString("这是写入的字符串")
	if err != nil {
		fmt.Println(err)
		return
	}
...
//优先选择：bufio方式计算Md5,尽量避免使用readall计算文件md5值（大文件会有内存分配问题）
